...
; (function () {
	...
	function Wtdapp() {
		...
		this.init = function () {
			...
			var events = document.getElementById('list');
		...
		this.CreateLogin = function () {
			...
			btnLogin.addEventListener('click', e => {
				...
				document.getElementById('melding').style.backgroundColor = '#C0392B';
			});

			btnLogout.addEventListener('click', e => {
				firebase.auth().signOut();
				document.getElementById('melding').innerHTML = "U bent uitgelogd";
				document.getElementById('melding').style.backgroundColor = '#6C7A89';
			...
			firebase.auth().onAuthStateChanged(firebaseUser => {
				if (firebaseUser) {
					...
					document.getElementById('melding').style.backgroundColor = '#26A65B';
				} else {
			...
			this.createEvents = function () {
				...
				// Create references
				var dbRefList = firebase.database().ref().child('evenementen');
			...
			this.CreateRegister = function () {
				...
				btnSignUp.addEventListener('click', e => {
					// Get email and pass
					document.getElementById('melding').style.backgroundColor = '#000';
					var email = txtEmail.value;
					var pass = txtPassword.value;
					var auth = firebase.auth();
					// Sign in
